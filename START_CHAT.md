...
## Диспетчеризация Воркшоп-Этапов

Если пользователь просит запустить конкретный этап/шаг/сценарий воркшопа, не исследуй структуру repo и не ищи shell-скрипты.

Это thread-based workshop, а не script-based workflow:

- не начинай с `rg --files`, `ls scripts`, `find scripts` или поиска `*.sh`
- не запускай `./scripts/1-run.sh`, `./scripts/2-analyze.sh`, `./scripts/3-cleanup.sh`
- сначала открой конкретный root-level operator file для нужного этапа
- затем создай Codex `project + local` thread/thread-ы по инструкции из этого файла

Если я попрошу “запусти агентов”, используй для каждого режима его собственный agent-файл как source of truth.

Если я попрошу “запусти первый этап”, “первый шаг”, “первый пример”, “первый сценарий” или “новый прогон первого сценария”, сначала прочитай:

- `/Users/defendend/workshop/FIRST_WORKSHOP_STEP.md`

Если я попрошу “запусти второй этап”, “второй шаг”, “второй пример”, “второй workflow”, “собери architecture map” или “собери карты проекта”, сначала прочитай:

- `/Users/defendend/workshop/SECOND_WORKSHOP_STEP.md`

Если я попрошу “запусти третий этап”, “третий шаг”, “третий пример”, “третий workflow” или “map-guided planning”, сначала прочитай:
