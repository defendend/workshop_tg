...
- для `ast-only`: `WORKFLOW.md` и `AST_AGENT.md`

Если работаешь в `ast-only` режиме, эти файлы нужно читать молча. Не пиши отдельные апдейты про чтение инструкций, локальные skills, проверку `HOME`, bootstrap, `DB_PATH`, `STATS_RESULT` или другие meta-блоки; в обычном прогоне нужен только финальный содержательный ответ, если нет anomaly.
...
5. Если работаешь в `ast-only` режиме:
...
- `ast-only` тоже можно запускать отдельным local project-thread
